  ...
  implements OnInit
{
  ...
  private routerParams$: Observable<RouterParams> = this.route
    .params as unknown as Observable<RouterParams>;
  ...
  private loadSideEffect() {
    this.hold(
      combineLatest([this.routerParams$, this.loadMovies$]).pipe(
  ...
  private errorSideEffect() {
    this.hold(this.select('error'), (error) => {
      if (error) {
        alert(error.message);
      }
    });
