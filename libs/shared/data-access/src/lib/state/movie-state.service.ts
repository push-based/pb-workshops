...
function compareFetchMoviePayload(
  oldVal: MovieFetchPayload,
  newVal: MovieFetchPayload
): boolean {
  ...
  return (
    (newVal as any).genreId === oldVal.genreId &&
...
export class MovieStateService
  extends RxState<MovieState>
  implements AbstractMovieState
{
  /** selectors **/
  ...
  private updateMovieRating$ = new Subject<{
    movie: MovieModel;
    rating: number;
  }>();
  ...
  loadMovieCategory(category: string) {
  ...
  updateMovieRating(payload: { movie: MovieModel; rating: number }) {
    this.updateMovieRating$.next(payload);
  }

  private fetchMoviesAction$ = (): Observable<MovieFetchPayload> =>
    merge(this.loadMovieCategory$, this.loadMovieGenre$);

  /** effect **/

  private fetchMovies$(
    payload: MovieFetchPayload
  ): Observable<
  ...
  > {
    ...
    return exhaustFetch(
      ...
      data$.pipe(map((response) => ({ movies: response.results })))
    );
  }
}
